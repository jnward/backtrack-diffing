...
with r1_model.trace("asdf") as tr:
...
# %%
# from crosscoder import BatchTopKSAE
...
if use_wandb:
    run = wandb.init(
        ...
        name=f"r1-sae-layer{layer_num}_{n_features}_{k}",
    )
...
# %%
from data_utils import sae_cached_activation_generator_padded, sae_new_cached_activation_generator
...
alpha = 1/32
sae_path = f"r1-sae-layer{layer_num}_{n_features}_{k}_multi"
...
my_data_generator = sae_cached_activation_generator_padded(
# my_data_generator = sae_new_cached_activation_generator(
    model=r1_model,
...
#         values = pre_acts * cc.W_decoder_FD.norm(dim=1)
...
for optimizer_step in pbar:
    ...
    for acc_step in range(accumulation_steps):
        ...
        features = sae.encode(target)
        reconstruction = sae.decode(features)
        error = target - reconstruction

        main_loss = F.mse_loss(reconstruction, target) / accumulation_steps
    ...
    if optimizer_step and optimizer_step % 3000 == 0 or optimizer_step == num_optimizer_steps-1:
        save_path = f"{sae_path}-{optimizer_step}.pt"
        print(f"Saving sae to {save_path}")
